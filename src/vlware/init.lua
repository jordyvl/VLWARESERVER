...
function bsodError(msg)
	...
	cprint(" VLWARE OS Server Edition ", 3)
	term.setBackgroundColor(colors.black)
	if term.isColor() then
		term.setBackgroundColor(colors.blue)
	end
...
systemDirs = {
	users = "/disk/users", -- users directory
	root = "/disk/SU", -- root directory (root USER)
...
if vlware_debug then
	clear()
	print("VLWARE OS Server Edition debug mode")
	shell.setDir(".vlware")
...
os.version = function()
	return config.read(nil, "oem") or "VLWARE OS 1.1 Server Edition"
end
shell.setDir("")
